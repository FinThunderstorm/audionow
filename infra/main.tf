...
resource "aws_s3_bucket" "app" {
  bucket = "audionow-alanendev-bucket"
  tags = {
    Environment = "production"
    Name        = "audionow-alanendev-bucket"
  }
}

resource "aws_s3_bucket" "logs" {
  bucket = "audionow-alanendev-logs-bucket"
  tags = {
    Environment = "production"
    Name        = "audionow-cf-logs"
  }
}
...
locals {
  s3_origin_id = "audionow-alanendev-bucket-origin-id"
}
...
resource "aws_cloudfront_distribution" "app" {
  ...
  comment             = "audionow-alanendev"
  default_root_object = "index.html"

  logging_config {
    include_cookies = false
    bucket          = aws_s3_bucket.logs.bucket_domain_name
    prefix          = "audionow-alanendev-cf"
  }
